  ...
  <view class="containe font_color">
		<view class="search flex_j_a_r">
			<view class="search_btn background">
				<input type="text" placeholder="搜索热搜" v-model="text" maxlength="20" confirm-type="search" @confirm="search_result({model:'input',t:text})" />
			</view>
		...
		<view class="v_result" v-show="show_state=='label'">
			...
			<view class="item" v-for="(item,index) in video_label" @click="search_result({model:'input',t:item.content})">
	  ...
	  <scroll-view v-show="show_state=='video'" class="video_set" scroll-y @scrolltolower="search_result">
		  <view class="noContent" v-if="videos.length <= 0">暂无内容</view>
		  <!-- <view class="passage"> -->
		  <view  v-for="(item,index) in videos" :key="index"  @click="detail(item)">
		  	<view class="person">
	  ...
	  <view v-show="show_state=='empty'">
		<view class="title flush_btn">热门搜索
			...
			<view class="hot_search">
				<view v-for="(item,index) in hotList" :key="index" @click="search_result({model:'click',t:item.title})" >{{item.title}}</view>
			</view>
	  </view>
  </view>
</template>
...
export default{
  ...
  setup(){
	...
	function search_result({model,t = ''}){
		...
		uni.request({
			...
			data:{
				...
				skip.value+=res.data.data.length
	...
	function search(e){
		...
		uni.request({
			method:"GET",
			url:uni.current_this.baseURL+':5001/get_moment_label_by_content',
			data:{
	...
	function fill(item){
		text.value=item
		search_result({model:'input',t:item})
		// search({detail:{
		// 	value:item
		// }})
	}
    ...
    return{text,input,search,record,fill,show_state,video_label,inter,search_result,videos,hotList,detail,user_info}
  }
}
</script>
